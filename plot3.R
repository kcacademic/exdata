plot3 <- function() {
  ...
  data$Date <- as.Date(data$Date, "%d/%m/%Y")
	...
	png('plot3.png')
	with(set, plot(set$Time, set$Sub_metering_1, type="l", xlab="", ylab="Energy sub metering"))
	with(set, points(set$Time, set$Sub_metering_2, type="l", col="red"))
	with(set, points(set$Time, set$Sub_metering_3, type="l", col="blue"))
